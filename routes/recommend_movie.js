...
router.route('')
    .get((req, res) => {
        ...
        let url;
        if (type == 'movie') {
            url = "https://api.themoviedb.org/3/movie/" + id + "/recommendations" + api_key + "&language=en-US&page=1";
        } else {
            url = "https://api.themoviedb.org/3/tv/" + id + "/recommendations" + api_key + "&language=en-US&page=1";
        }
        axios.get(url)
            .then(response => {
                let result = {},
